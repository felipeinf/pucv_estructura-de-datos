/*--------------------------------------------------------------------------------------------------------------------------
  Descripcion: Arboles binarios de busqueda.
...
/*-----------------------------------------------------------Recorridos de arboles----------------------------------------------------------
    ...
    Los recorridos pueden ser usados para buscar, mostrar, eliminar ,etc.
    
    **Para el CERT 3 deben saber; recorrido iterativo [pre|in|post] orden y recursivo [pre|in|post] orden.
    
      Ademas deben estudiar las pilas (push y pop) que se usan en los recorridos iterativos de arboles.
      
      Ejemplo de pila:
      
      struct Pila
      {
          struct NodoElemento *nodo;
          struct Pila *sig;
      };
      
      Funcion push: Inserta un nuevo nodo elemento en la cima de la pila, retorna 1 si lo hace con exito y 0 en caso contrario.
                    Tambien podria ser void y no retornar nada.
      
          int apilarNodoElemento( struct Pila **cumbrePila, struct NodoElemento *nuevo );
                      
  
	  Funcion pop: Retorna el ultimo elemento que fue insertado en la pila.
          
          struct NodoElemento *desapilarNodoElemento( struct Pila **cumbrePila );
      ...
      while( nodoActual != NULL ) //Esto tambien deben rutearlo y hacer un dibujo.
      {
        ...
        else
        {
          ...
          else
          {
            ...
            else nodoTemporal = nodoActual->izq; //Aca lo mismo:  for( nodoTemporal = nodoActual->izq;  nodoTemporal->der != NULL;  nodoPadre = nodoTemporal, nodoTemporal = nodoTemporal->der );
